...
export default function Programs() {
  ...
  const handleRenew = async (program: Program) => {
    if (!program.expiresAt) return

    setRenewingId(program.id)
    setError(null)
    try {
      const renewedProgram = await renewOnlineProgram(
        program.id,
        program.expiresAt,
      )
      setPrograms((current) =>
        current.map((item) =>
          item.id === program.id
            ? { ...item, expiresAt: renewedProgram.expiresAt, isActive: true }
            : item,
        ),
      )
      setConfirmRenewId(null)
      setRenewedId(program.id)
      window.setTimeout(() => setRenewedId(null), 3000)
    } catch (renewError) {
      setError(
        renewError instanceof Error
          ? `Programm konnte nicht verlängert werden: ${renewError.message}`
          : "Programm konnte nicht verlängert werden.",
      )
    } finally {
      setRenewingId(null)
    }
    ...
    <div>
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500">
            {loading
              ...
              : `${programs.length} Online-Programme`}
          </p>
        </div>
        <button
          type="button"
      ...
      {!loading && programs.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white py-20 text-center">
          <svg
            className="mx-auto mb-4 h-14 w-14 text-slate-200"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <p className="mb-1 font-medium text-slate-400">
            Noch keine Online-Programme
          </p>
          <p className="text-sm text-slate-400">
            Markieren Sie veröffentlichte Online-Übungen in der Bibliothek.
          </p>
        </div>
      ) : (
              ...
              >
                <div className="p-4 sm:p-5">
                  <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                    <div>
                      ...
                      <div className="mt-1 text-sm text-slate-500">
                        ...
                        {program.expiresAt &&
                          ` · gültig bis ${formatDate(program.expiresAt)}`}
                      </div>
                      {program.expiresAt && !isExpired && (
                        <div className="mt-1 text-xs font-medium text-teal-700">
                          Noch {remainingDays(program.expiresAt)} Tage gültig
                        </div>
                      )}
                      {renewedId === program.id && (
                        <div className="mt-1 text-xs font-medium text-teal-700">
                    ...
                    <div className="flex items-center gap-2">
                      <div>
                        ...
                        <div className="break-all font-mono text-lg font-bold tracking-wider text-teal-700">
                          {program.accessCode}
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={() => void copyCode(program)}
                  ...
                  <div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    >
                      {isExpanded ? "▾" : "▸"} {assignedExercises.length} Übung
                      {assignedExercises.length !== 1 ? "en" : ""}
                    </button>
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2">
                      {isConfirmingRenew ? (
                        <>
                          ...
                          <button
                            type="button"
                            onClick={() => void handleRenew(program)}
                            disabled={renewingId === program.id}
                            className="rounded-lg bg-teal-600 px-3 py-1.5 text-sm text-white hover:bg-teal-700 disabled:opacity-60"
                          >
                            {renewingId === program.id
                              ? "Wird verlängert …"
                              : "Ja, 6 Monate verlängern"}
                          </button>
                          <button
                            type="button"
                            onClick={() => setConfirmRenewId(null)}
                            disabled={renewingId === program.id}
                            className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm hover:bg-slate-50 disabled:opacity-60"
                          >
                            Abbrechen
                          </button>
                        </>
                          ...
                          >
                            {exercise.thumbnailUrl ? (
                              <img
                                ...
                                alt={exercise.title}
                                className="h-10 w-10 flex-shrink-0 rounded-lg bg-teal-50 object-cover"
                              />
                            ) : (
