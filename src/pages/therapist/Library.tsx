...
import type { Exercise, Page, Program } from "../../types"
import {
  createOnlineProgram,
      ...
      <div className="relative">
        ...
        {selectable ? (
          <button
            ...
            title={
              selected ? "Abwählen" : "Für das Patientenprogramm auswählen"
            }
          >
            {selected && (
              <svg
    ...
    <div>
      ...
      {onlineLoadError && (
        <div className="mb-5 flex flex-col gap-3 rounded-xl bg-red-50 p-3 text-sm text-red-700 sm:flex-row sm:items-center sm:justify-between">
          <span>
            Online-Übungen konnten nicht geladen werden: {onlineLoadError}
          </span>
          <button
            type="button"
        ...
        <div className="mb-4 bg-teal-600 text-white rounded-2xl p-4 flex items-center justify-between gap-4">
          <div>
            <div className="font-medium">
              Bezahlt – Programm für 6 Monate freigeschaltet!
            </div>
            <div className="text-teal-100 text-sm">
              Zugangscode für die Patientin / den Patienten:{" "}
              <span className="font-mono font-bold text-white text-lg">
                {createdProgram.accessCode}
              </span>
            </div>
            {createdProgram.expiresAt && (
              <div className="mt-1 text-xs text-teal-100">
                Gültig bis{" "}
                {new Date(createdProgram.expiresAt).toLocaleDateString("de-DE")}
              </div>
            )}
          </div>
          <div className="flex gap-2">
            <button
              onClick={() => copyCode(createdProgram.accessCode)}
              className="bg-white/20 hover:bg-white/30 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors"
            >
              Kopieren
            </button>
            <button
              onClick={() => setCreatedProgram(null)}
      ...
      <p className="-mt-2 mb-5 text-xs text-slate-500">
        Für Patientenprogramme können veröffentlichte Online-Übungen mit dem
        Kreis markiert werden. Lokale Beispiele und Entwürfe bleiben
        ausgeschlossen.
      </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-3xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-slate-500 mb-5">
              ...
              {selectedIds.size !== 1 ? "en" : ""} ausgewählt.
            </p>
            <div className="mb-5 rounded-2xl bg-teal-50 p-4 text-sm text-teal-900">
              <div className="font-semibold">6 Monate Zugang: 24 €</div>
